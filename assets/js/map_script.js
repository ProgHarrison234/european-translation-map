    ...
    function translate() {
        ...
        for (let i = 0; i < langList.length; i++) {
            ...
            requestList.push($.ajax(settings).done(function (response) {
                let tooltip = $("<span>").attr("data-tooltip", "").attr("tabindex", "1").attr("title", keymap[i]);
                console.log($("span"));
                ...
                marker.setPopup(popup).togglePopup();
                //tootip - tooltip(keymap[i])

                // <span data-tooltip tabindex="1" title="Fancy word for a beetle."> ... </span>


                
                //$(".translatedPopup").last().append(tooltip);

                /*Jwork
                <span data-tooltip tabindex="1" title="Fancy word for a beetle.">scarabaeus</span>
                */
                // check if the all api calls are done 
